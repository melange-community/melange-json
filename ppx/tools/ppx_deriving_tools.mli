...
val register_combined :
  ?deps:Deriving.t list -> label -> deriving list -> Deriving.t
...
class virtual deriving0 : object
  ...
  method derive_of_tuple : loc:location -> core_type list -> expression
  ...
  method derive_type_ref :
    loc:location -> label -> longident loc -> core_type list -> expression

  (** LOW-LEVEL METHODS *)

  method derive_of_core_type : core_type -> expression

  method derive_of_type_declaration :
    type_declaration -> value_binding list
end
