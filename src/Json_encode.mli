...
(** [pair encoder encoder tuple] creates a JSON array from a tuple of size
    2 *)
...
(** [array encoder l] makes a JSON array of the [list] [l] using the given
    [encoder] * NOTE: This will be renamed `array` once the existing and
    deprecated `array` function * has been removed. *)

val list : 'a encoder -> 'a list encoder
(** [list encoder a] makes a JSON array of the [array] [a] using the given
    [encoder] *)
